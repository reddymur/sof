...
int tplg_register_asrc(struct tplg_context *ctx)
{
	...
	if (tplg_create_controls(widget->num_kcontrols, ctx->file, NULL) < 0) {
...
int tplg_register_buffer(struct tplg_context *ctx)
{
	...
	if (tplg_create_controls(ctx->widget->num_kcontrols, ctx->file, NULL) < 0) {
		fprintf(stderr, "error: loading controls\n");
		return -EINVAL;
	}
...
int tplg_register_mixer(struct tplg_context *ctx)
{
	...
	ret = tplg_new_mixer(ctx, &mixer, NULL);
	if (ret < 0)
		return ret;
...
int tplg_register_pga(struct tplg_context *ctx)
{
	...
	ret = tplg_new_pga(ctx, &volume, NULL);
	if (ret < 0) {
